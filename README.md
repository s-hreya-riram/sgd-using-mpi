...
The two files referenced above are generated for each learning rate chosen and then all the cells of the Jupyter notebook `inference.ipynb` (this notebook may require updates to the log folders as necessary) can be run to generate the plots and the top 5 models for each learning rate.
